...
public class MainGui implements MemoryEventListener, RegisterEventListener,
	...
	@Override
	public int invokeDeviceInputEvent(final int devid) {
		// int data = 0;
		// switch (devid) {
		// case 0: // keyboard
		...
		// data += ((this._cardReaderBuffer[this._cardReaderBufferPos] << 8) &
		// 0xFFFF);
		// this._cardReaderBufferPos += 1;
				...
				switch (devid) {
					case 0: // keyboard
						String s = JOptionPane
								.showInputDialog("Please input: ");
						while (s != null && s.trim().length() == 0)
							s = JOptionPane.showInputDialog("Please input: ");
						
						txtInputView.append(s + "\n");
						Short number = null;
						try {
							number = Short.valueOf(s);
						} catch (NumberFormatException e) {
						...
						if (number != null) {
							...
							buffer[1] = (byte) ((number & 0xFF00) >> 8);
						} else {
							s = s.trim();
							buffer = s.getBytes();
						}
						_controller.setInputBuffer(buffer, devid);
						break;
					case 2: // card reader
						try {
							buffer = BinaryFileReader.toByteArray(_programFile
									.getAbsolutePath());
						} catch (FileNotFoundException e) {
							e.printStackTrace();
						} catch (IOException e) {
...
//		}
//		return status;
		 return this._controller.checkBuffer(devid);
	}
